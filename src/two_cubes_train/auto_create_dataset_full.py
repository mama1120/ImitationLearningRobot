...
def stack_cubes(bullet_client, robot, gripper, urdf_template, cube_positions, cube_sizes, cube_colors, env, dataset):
    ...
    for i, cube_id in enumerate(cube_ids):
        ...
        #Create some sort of "noise" in the pre-grasp position
        #random_offset1 = np.random.uniform(-0.03, 0.03)
        #random_offset2 = np.random.uniform(-0.03, 0.03)
        #random_offset3 = np.random.uniform(-0.03, 0.03)
        #pre_grasp_offset = Affine(translation=[random_offset1, random_offset2, -0.35+random_offset3])
