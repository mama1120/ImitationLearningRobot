...
def stack_cubes(bullet_client, robot, gripper, urdf_path, cube_positions, cube_orientations, cube_sizes, cube_colors, env, dataset):
    ...
    for i, cube_id in enumerate(cube_ids):
        ...
        env.save_demonstration(demo_folder, f"demo_{len(dataset)+410}_cube{i}_0pre_grasp.json", ACTIONS["move_to_pre_grasp"], i)
        ...
        env.save_demonstration(demo_folder, f"demo_{len(dataset)+410}_cube{i}_1move_to_grasp.json", ACTIONS["move_to_grasp"], i)
        ...
        env.save_demonstration(demo_folder, f"demo_{len(dataset)+410}_cube{i}_2lift.json", ACTIONS["lift_cube"], i)
        ...
        env.save_demonstration(demo_folder, f"demo_{len(dataset)+410}_cube{i}_3move_to_stack.json", ACTIONS["move_to_stack_position"], i)
        ...
        env.save_demonstration(demo_folder, f"demo_{len(dataset)+410}_cube{i}_4stack.json", ACTIONS["stack_cube"], i)
        current_translation = stack_target.translation
        current_translation[0] += generate_action_noise(std_dev=0.003)[0]  # Add noise to X
        current_translation[1] += generate_action_noise(std_dev=0.003)[1]  # Add noise to Y
        ...
        env.save_demonstration(demo_folder, f"demo_{len(dataset)+410}_cube{i}_5above_stack.json", ACTIONS["above_stack"], i)
        robot.lin(above_stack)

        # Return to home
        env.save_demonstration(demo_folder, f"demo_{len(dataset)+410}_cube{i}_6home.json", ACTIONS["return_home"], i)
