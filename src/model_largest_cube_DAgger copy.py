...
def train_online(model, dataset, input_scaler, output_scaler_position, output_scaler_orientation, epochs=2):
    ...
    model.save("updated_robot_stacking_model.keras")            #break
    training_cycles += 1
    time.sleep(3)
...
def train_online_bin(model, dataset, input_scaler, output_scaler_position, output_scaler_orientation, epochs=2):
    ...
    model.save("updated_robot_stacking_model.keras")
    training_cycles += 1
    time.sleep(3)
    print("Model saved as updated_robot_stacking_model.keras.")
    print("Training cycles:", training_cycles)

    return model

def stack_cubes(bullet_client, robot, gripper, urdf_path, cube_positions, cube_sizes, cube_colors, env, dataset, model):
...
# Create an empty dataset to store the demonstrations
    ...
    while True:
        ...
        if os.path.exists("updated_robot_stacking_model.keras"):
            ...
            model = load_model("Other_BC.keras")
