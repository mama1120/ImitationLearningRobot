...
def stack_cubes(
    ...
    for i, cube_id in enumerate(cube_ids):
        ...
        pre_grasp_pose = Affine(translation=current_translation, rotation=pre_grasp_pose.rotation)
        ...
        current_translation += generate_action_noise(std_dev=0.01)  # Add noise to pre-grasp position
        lift_pose = Affine(translation=current_translation, rotation=lift_pose.rotation)
        robot.lin(lift_pose)
        ...
        above_stack = Affine(translation=current_translation, rotation=stack_target.rotation)
